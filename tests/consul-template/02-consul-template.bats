...
@test "Check 'zookeeper' services declared in Consul are also declared in configuration" {
  ...
  zookeeper_id="1"
  zookeeper_ip="192.3.4.4"

  # Register a zookeeper server
  run register_key "service/zookeeper/servers/${zookeeper_id}" "${zookeeper_ip}"
  [ "$status" -eq 0 ]
  ...
  # Check we can't find the service registered (standalone mode)
  status=$(grep "server.${zookeeper_id}=${zookeeper_ip}:2888:3888" /conf/zoo.cfg 2>&1 > /dev/null;echo $?)
  [ "$status" -eq 1 ]

  # Register another zookeeper server
  run register_key "service/zookeeper/servers/2" "192.3.4.5"
  ...
  # Check we can find the service registered
  status=$(grep "server.2=192.3.4.5:2888:3888" /conf/zoo.cfg 2>&1 > /dev/null;echo $?)
  [ "$status" -eq 0 ]
