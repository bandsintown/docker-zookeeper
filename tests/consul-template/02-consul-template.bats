...
@test "Check 'zookeeper' services declared in Consul are also declared in configuration" {
  ...
  # Register a single zookeeper service
  run register_service $service
  [ "$status" -eq 0 ]

  # Assign the zookeeper id to the service registered
  run register_key "service/zookeeper/servers/${address}/id" "${zookeper_id}"
  [ "$status" -eq 0 ]
  ...
  # Check we are specifying the standalone mode is not enabled
  status=$(grep "standaloneEnabled=false" /conf/zoo.cfg 2>&1 > /dev/null;echo $?)
  [ "$status" -eq 0 ]
